...
RandWriter::RandWriter(const std::string& text, size_t k) {
  if (text.length() < k) {
    throw std::invalid_argument("Error: text has to have length at least k");
  }
  // read in text char by char
  std::string kgram;
  order = k;
  original = text;
...
size_t RandWriter::orderK() const { return order; }
...
// Overload the stream insertion operator << and display the iffnternal state
// of the Markov model.  Print out the order, alphabet, and the frequencies
// of the k-grams and k+1-grams
std::ostream& operator<<(std::ostream& os, RandWriter& rw) {
